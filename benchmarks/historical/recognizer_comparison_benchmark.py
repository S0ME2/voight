...
def main() -> int:
    ...
    from benchmarks.maintained.text_recognition_batch_benchmark import collect_lines
            ...
            for batch_size in args.batch_sizes:
                for _ in range(args.repeats):
                    try:
                        ...
                        rows.append(Row("paddle", name, str(model_path), strategy, batch_size, "failed", len(lines), None, None, None, [], None, None, None, None, None, len(lines), f"{type(error).__name__}: {error}"))
    output = args.output_dir or ROOT / "benchmarks/results" / f"recognizers-{datetime.now():%Y%m%dT%H%M%S}"

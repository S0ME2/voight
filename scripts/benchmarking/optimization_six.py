...
def experiment_five() -> int:
    ...
    for kind, variant, count, role in (("passport", "passport_mrz_only", 2, "image"), ("id_card", "id_card_mrz_known_back", 3, "back")):
        ...
        for document in corpus:
            ...
            crop, _ = __import__("scripts.benchmarking.pipeline_breakdown", fromlist=["crop_polygon"]).crop_polygon(image, location.polygon.reshape(4, 2), settings.mrz.polygon_padding_ratio)
            processed = __import__("scripts.benchmarking.pipeline_breakdown", fromlist=["preprocess"]).preprocess(crop, settings.mrz.max_side, settings.mrz.contrast)

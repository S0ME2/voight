...
PRIMARY_CONFIG = {
    ...
    "TEXT_RECOGNITION_PACKING": "aspect-ratio", "TEXT_DETECTOR_MODEL": "PP-OCRv6_medium_det",
    "TEXT_RECOGNIZER_MODEL": "PP-OCRv6_medium_rec", "MRZ_RECOGNIZER_BACKEND": "generic-paddle",
...
def _stage_values(diagnostics: dict[str, Any], total: float) -> dict[str, float]:
    ...
    values = {
        "localization": localization, "canonicalization": float(pipeline.get("canonicalization_seconds", 0.0)),
        "data_crop": float(pipeline.get("data_crop_seconds", 0.0)), "mrz_crop_preprocess": float(pipeline.get("mrz_crop_preprocess_seconds", 0.0)),
        ...
        "assembly": float(pipeline.get("result_assembly_seconds", 0.0)) + float(diagnostics.get("result_unpack_seconds", 0.0)),
    }
...
def run_partial(settings: Settings, models: Models, documents: list[Document], kind: str, variant: str) -> tuple[float, dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    for job in jobs:
        ...
        if visible_result and job.item_id in prepared:
            ...
            values, _ = complete_profile(prepared[job.item_id], tokens, job.parse_fields, job.validate_fields, min_overlap=job.min_overlap, ocr_seconds=None)[0:2]
    ...
    diagnostics = {"localization": localization_diag, "pipeline": {"document_preparation_seconds": prep_seconds, "canonicalization_seconds": sum(p.timings.get("canonicalization_seconds", 0.0) for p in prepared.values()), "data_crop_seconds": sum(p.timings.get("data_crop_seconds", 0.0) for p in prepared.values()), "mrz_crop_preprocess_seconds": mrz_crop_seconds, "result_assembly_seconds": time.perf_counter() - assembly_started}, "variant_scope": variant}

...
def _profile() -> dict[str, int] | None:
    return {} if os.getenv("VOIGHT_BENCHMARK_PROFILE") else None


def _add_ns(profile: dict[str, int] | None, name: str, started_ns: int) -> None:
    if profile is not None:
        profile[name] = profile.get(name, 0) + time.perf_counter_ns() - started_ns


async def _read(file: UploadFile, settings: Settings, profile: dict[str, int] | None = None) -> UploadedDocument:
    ...
    _add_ns(profile, "api.upload_read_ns", started)
    return document
...
def _safe_entries(archive: UploadedDocument, settings: Settings, profile: dict[str, int] | None = None) -> list[UploadedDocument]:
    started = time.perf_counter_ns()
    ...
    try:
        with zipfile.ZipFile(BytesIO(archive.data)) as zipped:
            ...
            for info in zipped.infolist():
                ...
                _add_ns(profile, "api.archive_entry_read_ns", entry_started)
                entries.append(document_from_bytes(entry_data, path.name, source_filename=archive.filename, archive_path=info.filename))
    ...
    _add_ns(profile, "api.archive_parse_ns", started)
    if profile is not None:
        profile["api.archive_entry_count"] = profile.get("api.archive_entry_count", 0) + len(entries)
    return entries


def _id_archive(archive: UploadedDocument, settings: Settings, profile: dict[str, int] | None = None) -> list[LogicalInput]:
    started = time.perf_counter_ns()
    pairs: dict[str, dict[str, UploadedDocument]] = {}
    for entry in _safe_entries(archive, settings, profile):
    ...
    _add_ns(profile, "api.id_archive_grouping_ns", started)
    return results
...
def _run_batch(inputs: list[LogicalInput], models: Models, settings: Settings, profile: dict[str, int] | None = None) -> OcrBatchResponse:
    if not inputs:
    ...
    if profile is not None:
        profile["api.logical_input_count"] = len(inputs)
    artifacts = create_batch_artifact_run(settings.artifacts, "v1", profile)
    _add_ns(profile, "api.artifact_batch_create_ns", profile_started_ns)
    request_save_started = time.perf_counter_ns()
    artifacts.save_json(
        "00_request.json",
        {"inputs": [logical.input.model_dump(mode="json") for logical in inputs]},
    )
    _add_ns(profile, "api.request_manifest_save_ns", request_save_started)
    planning_started = time.perf_counter()
    plan = build_batch_plan(inputs, settings, artifacts, profile)
    ...
    try:
        ...
        _add_ns(profile, "api.model_pipeline_ns", runner_started)
        if profile is not None:
            profile.update(diagnostics.get("benchmark_profile", {}))
    ...
    _add_ns(profile, "api.response_assembly_ns", assembly_started)
    if profile is not None:
        response.diagnostics["benchmark_profile"] = profile
    diagnostics_save_started = time.perf_counter_ns()
    artifacts.save_json("01_pipeline_diagnostics.json", response.diagnostics)
    _add_ns(profile, "api.pipeline_diagnostics_save_ns", diagnostics_save_started)
    response_save_started = time.perf_counter_ns()
    artifacts.save_json("02_response.json", response.model_dump(mode="json"))
    _add_ns(profile, "api.response_artifact_save_ns", response_save_started)
    _add_ns(profile, "api.endpoint_work_ns", profile_started_ns)
    return response
...
def create_v1_router(settings: Settings, models: Models) -> APIRouter:
    ...
    async def run(inputs: list[LogicalInput], profile: dict[str, int] | None = None) -> OcrBatchResponse:
        try:
            return await gate.run(lambda: _run_batch(inputs, models, settings, profile))
        except QueueFullError as exc:
            raise _error(ErrorCode.QUEUE_FULL, str(exc), 503) from exc
    ...
    async def passport(image: UploadFile = File(...)) -> OcrResponse:
        profile = _profile()
        return _single(await run(_image_inputs(DocumentType.PASSPORT, [await _read(image, settings, profile)]), profile))
    ...
    async def id_card(front: UploadFile = File(...), back: UploadFile = File(...)) -> OcrResponse:
        profile = _profile()
        files = (await _read(front, settings, profile), await _read(back, settings, profile))
        response = await run([LogicalInput(DocumentInput(document_type=DocumentType.ID_CARD, front=files[0].filename or "front", back=files[1].filename or "back"), files)], profile)
    ...
    async def driving_license(image: UploadFile = File(...)) -> OcrResponse:
        profile = _profile()
        return _single(await run(_image_inputs(DocumentType.DRIVING_LICENSE, [await _read(image, settings, profile)]), profile))
    ...
    async def passport_batch(images: BatchUploads = [], archive: OptionalUpload = None) -> OcrBatchResponse:
        profile = _profile()
        files = [await _read(file, settings, profile) for file in images]
        if archive is not None:
            files.extend(_safe_entries(await _read(archive, settings, profile), settings, profile))
        return await run(_image_inputs(DocumentType.PASSPORT, files), profile)
    ...
    async def id_card_batch(archive: UploadFile = File(...)) -> OcrBatchResponse:
        profile = _profile()
        return await run(_id_archive(await _read(archive, settings, profile), settings, profile), profile)
    ...
    async def driving_license_batch(images: BatchUploads = [], archive: OptionalUpload = None) -> OcrBatchResponse:
        ...
        if archive is not None:
            files.extend(_safe_entries(await _read(archive, settings, profile), settings, profile))
        return await run(_image_inputs(DocumentType.DRIVING_LICENSE, files), profile)
